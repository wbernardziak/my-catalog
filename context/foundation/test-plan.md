...
**Amended 2026-09-14** (`context/archive/2026-09-14-testing-guard-self-verification/research.md`).
Phase 6 research re-probed Risk #10 and corrected its response guidance in three places;
the risk row itself is unchanged.

- **Empty-scan floors fit two guards, not three.** The colour-literal and `games`-read
  scanners pass on an empty tree; the contrast guard already fails on any missing theme
  block or token. Its silent gap is a hand-kept theme list that a new theme would bypass.
- **The colour hole is wider than the palette.** Tailwind's logical border roots
  (`border-s`, `border-e`, `border-bs`, `border-be`) are missing alongside the four
  newer palette families, so "palette tracks Tailwind" alone would leave it open.
- **`bash -n` is the weakest hook check.** The script parses today; a dropped
  registration or a lost exec bit is the likelier break and costs the same to assert.

The refresh research also over-stated one games-guard bypass: a misleading comment fools
the guard only when it sits inside the query statement, not after its terminating `;`.
...
| #10  | Each guard exits non-zero on a known-bad fixture, naming the violation rather than failing for an unrelated reason; the two source scanners (colour literals, `games` reads) also exit non-zero on an empty or zero-match scan, while the contrast guard — which already fails closed on missing blocks — proves instead that its theme list tracks the app's theme registry; the colour guard's palette **and its colour-utility roots** track the installed Tailwind; removing a gate step from the CI workflow, a guard script from `package.json`, the commit-hook wiring or the Stop-hook registration fails `npm test`; the Stop hook stays registered, executable and parseable (`bash -n`) | That the gate exists, therefore it runs. That exit 0 means clean. That `.mjs` is unlinted — CI lints it; the real gaps are guard logic and wiring. That an empty-scan floor is the right fix for every guard — the contrast guard's silent gap is list drift, not an empty scan. That tracking Tailwind's palette closes the colour hole — the utility roots lag too                                                                                                                                                                         | Each guard's scan root, file floor and hardcoded lists, and how a fixture root can be injected; which gate steps the workflow and `package.json` must carry                                                                                                | Unit self-tests that execute each guard against a temporary fixture tree, plus a text assertion over the workflow and scripts, and a registration/exec-mode check on the Stop hook — `bash -n` alone is the syntax layer only, not the likelier break | Widening lint globs or adding shellcheck and calling it done without a deliberate-break check; a fixture that fails for the wrong reason                                                                                                                                   |
...
| 6   | Guard self-verification             | Prove every guard and gate fails on a known-bad input and on an empty scan, tracks the installed tooling, and cannot be dropped from CI unnoticed                                 | #10           | unit self-tests over fixture trees + workflow/script wiring assertion + `bash -n` | complete    | `context/archive/2026-09-14-testing-guard-self-verification/`       |
...
| existing suite              | —                                            | —                                      | 24 files: 19 in the `unit` project (7 in `src/lib/services/`, 5 API-route, 4 guard and wiring self-tests in `scripts/`, plus theme, types, and one component test — `GameForm.test.ts`, which exercises `fromRow`, not rendering) and 5 in `db`. 212 unit tests + 35 db tests as of 2026-09-14 (§3 Phase 6).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| guard self-tests                 | local + CI (`npm test`)                         | required (wired 2026-09-14)                                                                               | a guard passing on an empty scan, a hardcoded list that fell behind the installed tooling, or a gate step removed from the CI workflow or `package.json`                                                                                                                                                                                            |
...
**Phase 6 (2026-09-14).** Every guard defect below was proven by a probe and
fixed in the guard itself before its self-test was written, so no test pins a
defect in place:

- `check-color-literals.mjs` passed an empty `src/`, lacked Tailwind 4.2's
  `mauve`, `mist`, `olive` and `taupe` families, and lacked the logical border
  roots `border-s/e/bs/be`. The palette is now read from the installed
  `tailwindcss/theme.css`.
- `check-games-read-guard.mjs` passed an empty or write-only tree, let a
  comment fake the predicate or a write verb, and missed `.from("games" as
const)` and `.from(<identifier>)`. The implementation review then found
  three more bypasses, also fixed: an indirect name on any camelCase receiver
  (`supabaseClient.from(T)`), a sibling query in the same statement lending its
  predicate or write verb (`Promise.all([...])`), and the comment blanker
  pairing two JSX apostrophes or reading `/\//` as a comment. The blanker now
  fails closed on both.
- `check-contrast.mjs` kept its theme and gradient lists by hand, so a fourth
  theme or a second `bg-clip-text` heading went unchecked. Both lists are now
  checked against the source.

The wiring test's first draft matched `ci.yml` as a whole file and read the
hook's mode from disk, so moving a step into the wrong job or clearing the
committed exec bit stayed green. Its deliberate-break rows (5.6, 5.7) caught
that before merge.

### 6.8 Proving a guard can fail

A guard that has never been seen to fail is not a gate. Every guard under `scripts/` gets a
self-test in `scripts/<guard>.test.ts`, and the patterns below are what make that test worth
having (worked examples: the three `check-*.test.ts` files).

- **Point the guard at a fixture with `--root=<absolute dir>`, never an environment variable.** A
  variable left set in a shell could silently aim CI at the wrong tree — the vacuous pass the test
  exists to rule out. The flag is opt-in, an unknown argument exits 1, and an overridden run logs
  `Scanned root:`.
- **Use `scripts/guardHarness.ts`.** `makeTree(files)` writes a fixture tree and returns its path;
  `runGuard(script, root)` spawns the guard with the repo root as `cwd`; `removeTree` runs in
  `afterAll`. Guards end in `process.exit`, so spawning is the only honest seam.
- **Fixtures live under the OS temp directory, never under `src/`.** A fixture in `src/` is scanned
  by the real guard run and by every other gate.
- **Assert the rule and the file, not only the exit status.** A fixture that exits 1 for an
  unrelated reason (a missing `src/`) looks identical to a real catch. Where a guard reports kinds,
  assert the exact set of `file:line kind` hits so a missing and an extra hit both fail.
- **Pair every known-bad tree with a known-good tree and an empty scan.** Known-good catches false
  positives on the house idiom; the empty or zero-match case proves the floor.
- **Take expected values from outside the guard:** Tailwind's own design system, `src/lib/theme.ts`,
  a copy of the real `global.css`, or hand-written snippets. An expected set copied from the
  guard's own list is an implementation mirror and cannot fail.
- **One spawn per tree.** Put many known-bad files in one tree and assert per file. A spawn costs
  about 70 ms, and `npm test` runs in CI, the commit hook and the `Stop` hook; the whole suite of
  guard tests is budgeted at 1.5 s, measured as the median of 3 × `/usr/bin/time -f %e npm test`.
- **Break it deliberately once.** Revert the fix or delete the rule, watch the named test go red
  with the expected message, then restore it — from a backup copy, not `git checkout`, if the fix
  is not yet committed.
- **Wire it in `scripts/gate-wiring.test.ts`.** A new guard adds its `npm run <script>` to the
  expected `run:` steps of the right `ci.yml` job, its script name and file to the `package.json`
  assertions, and, if a hook runs it, that hook's assertion. Then delete the new step from `ci.yml`
  once and watch the wiring test go red.
...
- **Lint coverage for `.sh` and lint-staged `.js`/`.mjs`** — the commit hook's
  lint-staged globs skip `.js`/`.mjs`, and no shellcheck runs over
  `.claude/hooks/*.sh`. Excluded because CI already runs `eslint .` over
  `scripts/**/*.mjs`, the wiring test asserts the hook's `bash -n` and loop
  guard, and adding lint coverage without a deliberate-break check is the
  anti-pattern §6.8 rules out. Re-evaluate if a hook-script or `.mjs` defect
  reaches `main` that neither `eslint .` nor `scripts/gate-wiring.test.ts`
  caught. (Source: impl-review F3,
  `context/archive/2026-09-14-testing-quality-gate-wiring/reviews/impl-review.md`.)
