...
| 2   | Per-member state attribution        | Prove played state and preference stay bound to the correct household member at both the query and the policy layer                                    | #1            | integration + DB-level RLS verification | complete                         | `context/changes/testing-per-member-state-attribution/`     |
