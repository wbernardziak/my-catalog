...
Use an explicit `--root` fixture seam, never an environment variable. Build fixture trees in
`guardHarness.ts` under the OS temp directory, assert both the rule/file output and exit status,
and include known-good plus empty-scan cases. Take oracles from Tailwind, the theme registry, or
the real stylesheet rather than a guard's own lists. Keep fixtures grouped per spawn, retain the
~1.5 s budget, deliberately break each new guard once, and add its runtime surface to
`scripts/gate-wiring.test.ts`.
