...
| CI                          | GitHub Actions                               | n/a                  | `.github/workflows/ci.yml` has two jobs: `ci` runs `typecheck`, lint, `lint:colors`, `lint:contrast`, build **and `npm test`**; `db-tests` (added 2026-09-12) boots a local stack and runs `npm run test:db`. The plain test step — the test step was already wired in commit `1b3980b`, before this plan was authored (corrected 2026-09-11). What Phase 5 still owes: dropping `--passWithNoTests`, and the local edit-loop gate.                                                                                                        |
