...
- **What this boundary guarantees is identity plus player count.** The catalog
  allow-list (`recommendations.ts:153-156`) and the player-count guard are
...
- **Prove the test can fail**: add `...g` to the prompt pick
  (`recommendations.ts:73`) — both route payload cases must redden; add `...row`
