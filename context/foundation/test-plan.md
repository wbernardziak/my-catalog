...
- **Location**: `src/test/db/` — a separate vitest project, not part of `npm test`.
  `writeOwn.test.ts` holds policy-layer assertions, `readAttribution.test.ts` holds
  application-layer ones. `src/test/db/README.md` states what the harness proves and
  what it costs.
