...
| 4   | LLM recommendation guardrails       | Prove recommendations stay inside the eligible catalog, fail visibly, and send only minimal data under adversarial provider responses                  | #3, #5, #7    | contract tests with stubbed provider                    | change opened                    | `context/changes/testing-llm-recommendation-guardrails/`            |
...
| existing suite              | —                                            | —                    | 18 files: 13 in the `unit` project (7 in `src/lib/services/`, 3 API-route, plus theme, types, and one component test) and 5 in `db`. 150 unit tests + 35 db tests as of 2026-09-13 (§3 Phase 3).                                                                                                                                                                                                                                                                                                                                           |
...
| DB / RLS verification       | Supabase CLI (local stack)                   | 2.23.x devDependency | `npx supabase start` requires Docker. **Used by `npm run test:db`** (the `db` vitest project under `src/test/db/`) since 2026-09-12 — the only suite that can observe RLS. `project_id` was `10x-astro-starter` (the starter default) until 2026-09-12, which collided with another local repo's stack: `supabase start` reused that project's containers, so this repo's migrations were never applied and every query hit a foreign schema. Now `my-catalog` on ports 54330-54339, with `SUPABASE_URL` in `.dev.vars` pointing at 54331. |
...
| CI                          | GitHub Actions                               | n/a                  | `.github/workflows/ci.yml` has two jobs: `ci` runs `typecheck`, lint, `lint:colors`, `lint:contrast`, `lint:reads` (added 2026-09-13), build **and `npm test`**; `db-tests` (added 2026-09-12) boots a local stack and runs `npm run test:db`. The test step was already wired in commit `1b3980b`, before this plan was authored (corrected 2026-09-11), and `--passWithNoTests` was dropped the same day, so an empty or uncollectable suite already fails CI. What Phase 5 still owes: the local edit-loop gate alone.                                                                                                        |
...
- **A new read path is not covered by any of this** — it is new code no existing
  ...
  check runs before the `.select(` check, because `createGame` (`games.ts:83`)
